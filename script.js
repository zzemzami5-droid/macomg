...
const setLang = (lng) => {
  ...
  langBtns.forEach(b => {
    const isActive = b.dataset.lang === lng;
...
const hamburger = document.querySelector('.hamburger-simple');
...
const langBtnsMenu = document.querySelectorAll('.lang-btn-menu-simple');

const setLangButtons = (lng) => {
  langBtnsMenu.forEach(b => {
    const isActive = b.dataset.lang === lng;
    b.classList.toggle('active', isActive);
    b.setAttribute('aria-pressed', String(isActive));
  });
...
langBtnsMenu.forEach(b => b.addEventListener('click', () => setLang(b.dataset.lang)));
setLang('en');
